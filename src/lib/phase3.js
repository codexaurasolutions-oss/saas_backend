...
export const resolveTemplateContext = async (salonId, context = {}) => {
  ...
  const resolved = {
    ...
    membership_name: membership?.membershipPlan?.name || "",
    membership_expiry: membership?.endsAt ? new Date(membership.endsAt).toLocaleDateString() : (resolvedCustomer?.memberships?.[0]?.endsAt ? new Date(resolvedCustomer.memberships[0].endsAt).toLocaleDateString() : templateFallbacks.membership_expiry),
